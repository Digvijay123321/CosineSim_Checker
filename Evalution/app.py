...
@app.route('/')
def index():
    """Render the main form where user inputs the company name."""
    return render_template('index.html')


@app.route('/submit', methods=['POST'])
def submit():
    """Handle form submission, save company info, and run the scripts."""
    company_name = request.form['company_name']
    print(company_name)
    
    
    # Run the scripts (evaluate_documents.py and evaluation.py)
    try:
        clear_temp_folder()                                                 # refresh temp
        print('\n\n\n')
        documents_download()
        print('\n\n\n')
        evaluate_similarity(company_name)
        print('\n\n\n')
        clear_temp_folder()                                                 # refresh temp
...
@app.route('/result')
def result():
    """Display the result after evaluation is complete."""
...
def documents_download():
    ...
    doc1_folder = "Evaluation/Doc1/"
    doc2_folder = "Evaluation/Doc2/"
...
def evaluate_similarity(company_name):
